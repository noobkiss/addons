function EsoZH_doubleNameSkills(EsoZH)
	if GetCVar("language.2") == "zh" or GetCVar("language.2") == "ze" then
		...
		local function modifyTooltip(isPassive, isNew, ...)
			...
			if EsoZH.Settings.SkillName ~= "zh" then
				...
				if finalName and zhName then
					zhName = string.gsub(zhName, "[%z\1-\127\194-\244][\128-\191]*", "") -- 中英去英文
		...
		--if EsoZH:IsAddonRunning("HarvensImprovedSkillsWindow") then
		--	if HarvensImprovedSkillsWindow then
		--		ZO_PreHook(HarvensImprovedSkillsWindow.AbilityOnMouseEnter, function() SafeAddString(SI_ABILITY_TOOLTIP_NAME, EsoZH.StringsBackup["SI_ABILITY_TOOLTIP_NAME"], 10) end)
		--		ZO_PreHook(HarvensImprovedSkillsWindow.AbilityOnMouseEnter, function() SafeAddString(SI_ABILITY_NAME_AND_RANK, EsoZH.StringsBackup["SI_ABILITY_NAME_AND_RANK"], 10) end)
		--	end
		--end

		----if HarvensImprovedSkillsWindow then
		----	abilityTooltipHook(HarvensSkillTooltipMorph1, "SetActiveSkill", false, false, false)
		----	abilityTooltipHook(HarvensSkillTooltipMorph2, "SetActiveSkill", false, false, false)
		----	--abilityTooltipHook(HarvensSkillTooltipMorph2, "SetActiveSkill", false, false, false)
		----end
