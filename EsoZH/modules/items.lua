function EsoZH_doubleNameItems(EsoZH)
	if GetCVar("language.2") == "zh" or GetCVar("language.2") == "ze" then
		...
		local function splitItemName(itemName, itemNameRaw)
			local finalName = ""
			if string.match(itemName, "%s*" .. itemNameRaw .. "%s*") then
				local prefix, affix = string.match(itemName, "^(.-)%s*" .. itemNameRaw .. "%s*(.-)$")
				...
				return finalName
			elseif string.match(itemName, "%s*" .. itemNameRaw .. "$") then
				local prefix = string.match(itemName, "^(.-)%s*" .. itemNameRaw .. "$")
				...
				return finalName
			elseif string.match(itemName, "^" .. itemNameRaw .. "%s*") then
				local affix = string.match(itemName, "^" .. itemNameRaw .. "%s*(.-)$")
		...
		local function modifyTooltip(lnk)
			if EsoZH.Settings.ItemDoubleName ~= EsoZH.dropDownparm["zh"] or EsoZH.Settings.ItemTraits ~= "zh" or EsoZH.Settings.EnchantsName ~= "zh" or EsoZH.Settings.SetsName ~= "zh" or EsoZH.Settings.ItemDoubleName ~= "zh" then
				...
				--d("itmNameRawZh: " ..tostring(itmNameRawZh))
				...
				elseif itmType == ITEMTYPE_POISON or itmType == ITEMTYPE_POTION then
					...
					elseif itemDataDicts.Potions[string.gsub(itmName, " ", "")] then               --中英模式匹配
						finalName = itemDataDicts.Potions[string.gsub(itmName, " ", "")]   --中英模式匹配
				...
				elseif itmType == ITEMTYPE_ARMOR or itmType == ITEMTYPE_WEAPON then
					...
					elseif itmNameRawZh and itemDataDicts.Parts[string.gsub(itmNameRawZh, "%^.*", "")] then										--中英模式匹配
						finalName = splitItemName(string.gsub(itmName, "%^.*", ""), string.gsub(itmNameRawZh, "%^.*", ""))							--中英模式匹配
					elseif itemDataDicts.Items[itmId] then
						finalName = itemDataDicts.Items[itmId]
					end
				...
				if itmType == ITEMTYPE_ARMOR or itmType == ITEMTYPE_WEAPON or itmType == ITEMTYPE_ARMOR_TRAIT or itmType == ITEMTYPE_JEWELRY_TRAIT or itmType == ITEMTYPE_WEAPON_TRAIT then
					...
					if itemDataDicts.Traits[traitType] then
						...
						zhTrait = string.gsub(trait, "[%z\1-\127\194-\244][\128-\191]*", "")
						--d(zhTrait)
