...
LibChat2 is a library which must be added as ## DependsOn: libChat2 or  ## OptionalDependsOn: libChat2 into an ESO Addon, throught the addons manifest txt file.
LibChat2 does not require LibStub to work anymore. It can be still used but it's recommanded to use the global LibChat2 instead
...
Minor = 10 : LibChat2 10	--Baertram (made standalone, added global LibChat2)
Minor = 11 : LibChat2 11	--Baertram (fixed messages format for EVENT_CHAT_MESSAGE_CHANNEL to use string.format now)
Minor = 12 : LibChat2 12	--Baertram (fixed usernames containing gender stuff like ^m)

More info : http://www.esoui.com/downloads/info740-libChat2.html

]]--

local LIB_NAME, LIB_VERSION = "libChat-1.0", 12 -- libChat2 is a copy of libChat-1.0 so it will use the same tag for LibStub so it is not accidently loaded with an older version from libChat1.0

local libchat, oldminor
if LibStub then
	libchat, oldminor = LibStub:NewLibrary(LIB_NAME, LIB_VERSION)
else
	libchat = {}
end
if not libchat then return end
LibChat2 = libchat
