...
function MasterMerchant:SetLinkStyle(itemLink, style)
  local f = ParseFields(itemLink)
  f[IDX.LINK_STYLE] = EnsureString(style or LINK_STYLE_DEFAULT)
  return BuildItemLinkFromFields(f)
end

function MasterMerchant:SetLevelAndSubType(itemLink)
  local f = ParseFields(itemLink)
...
function MasterMerchant:CheckLevelAndSubType(itemLink)
  local f = ParseFields(itemLink)
...
function MasterMerchant:GenerateStatsItemTooltip()
  ...
  elseif mocParentName == "ZO_FurnitureVaultListContents" then
    ...
    itemLink = MasterMerchant:CheckLevelAndSubType(rowItemLink)
  ...
  elseif mocParentName == "FurCGui_ListHolder" then
    if mouseOverControl.itemLink then
      itemLink = MasterMerchant:CheckLevelAndSubType(mouseOverControl.itemLink)
    end
...
function MasterMerchant:ToggleVendorMarker(rowControl, slot)
  ...
  local anchorPoint, relAnchorPoint
  if AwesomeGuildStore then
    anchorPoint, relAnchorPoint = RIGHT, LEFT
  else
    anchorPoint, relAnchorPoint = LEFT, LEFT
  end
  ...
  if (not markerControl) then
    ...
    markerControl:SetDimensions(22, 22)
    markerControl:SetInheritScale(false)
    markerControl:SetAnchor(anchorPoint, sellPriceControl, relAnchorPoint)
    markerControl:SetDrawTier(DT_HIGH)
  end
