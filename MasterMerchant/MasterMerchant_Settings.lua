...
local function CheckAGSPriceValue()
  if MasterMerchant.systemSavedVariables.agsSalePriceToUse ~= MM_PRICE_TTC_SUGGESTED then
    MasterMerchant.systemSavedVariables.modifiedSuggestedAgsSalePrice = false
  end
end
...
function MasterMerchant:SetupOptionsData()
  ...
  optionsData[#optionsData + 1] = {
    type = 'dropdown',
    name = GetString(MM_AGS_TRADER_VALUE_TYPE_NAME),
    tooltip = GetString(MM_AGS_TRADER_VALUE_TYPE_TIP),
    choices = MasterMerchant.dealCalcChoices,
    choicesValues = MasterMerchant.dealCalcValues,
    getFunc = function() return MasterMerchant.systemSavedVariables.agsSalePriceToUse end,
    setFunc = function(value)
      MasterMerchant.systemSavedVariables.agsSalePriceToUse = value
      CheckAGSPriceValue()
    end,
    default = MasterMerchant.systemDefault.agsSalePriceToUse,
    disabled = function() return not MasterMerchant.AwesomeGuildStoreDetected end,
  }
  optionsData[#optionsData + 1] = {
    type = "header",
